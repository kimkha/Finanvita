...
package com.kimkha.finance.ui.settings.lock;
...
/**
 * Displays and detects the user's unlock attempt, which is a drag of a finger across 9 regions of the screen. Is also capable of displaying a static pattern in
 * "in progress", "wrong" or "correct" states.
 */
public class LockPatternView extends View
{
    ...
    /**
     * How many milliseconds we spend animating each circle of a lock pattern if the animating mode is set. The entire animation should take this constant * the
     * length of the pattern to complete.
     */
    private static final int MILLIS_PER_CIRCLE_ANIMATING = 700;

    private OnPatternListener listener;
    private ArrayList<Cell> patternList = new ArrayList<Cell>(9);

    /**
     * Lookup table for the circles of the pattern we are currently drawing. This will be the cells of the complete pattern unless we are animating, in which
     * case we use this to hold the cells we are drawing for the in progress animation.
     */
    ...
    public LockPatternView(Context context, AttributeSet attrs)
    {
        ...
        pathPaint.setAlpha(mStrokeAlpha);
        pathPaint.setStyle(Paint.Style.STROKE);
        pathPaint.setStrokeJoin(Paint.Join.ROUND);
        pathPaint.setStrokeCap(Paint.Cap.ROUND);

        // lot's of bitmaps!
        bitmapBtnDefault = getBitmapFor(R.drawable.alp_btn_code_lock_default_holo);
        bitmapBtnTouched = getBitmapFor(R.drawable.alp_btn_code_lock_touched_holo);
        bitmapCircleDefault = getBitmapFor(R.drawable.alp_indicator_code_lock_point_area_default_holo);
        bitmapCircleGreen = getBitmapFor(R.drawable.alp_indicator_code_lock_point_area_green_holo);
        bitmapCircleRed = getBitmapFor(R.drawable.alp_indicator_code_lock_point_area_red_holo);

        bitmapArrowGreenUp = getBitmapFor(R.drawable.alp_indicator_code_lock_drag_direction_green_up);
        bitmapArrowRedUp = getBitmapFor(R.drawable.alp_indicator_code_lock_drag_direction_red_up);

        // bitmaps have the size of the largest bitmap in this group
        final Bitmap bitmaps[] = {bitmapBtnDefault, bitmapBtnTouched, bitmapCircleDefault, bitmapCircleGreen, bitmapCircleRed};
    ...
    protected Parcelable onSaveInstanceState()
    {
        ...
        return new SavedState(superState, LockPatternUtils.patternToString(patternList), mPatternDisplayMode.ordinal(), mInputEnabled, mInStealthMode,
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        viewWidth = viewHeight = Math.min(Math.min(viewWidth, viewHeight), context.getResources().getDimensionPixelSize(R.dimen.lockpatternview_max_size));
    ...
    {
        final ArrayList<Cell> pattern = patternList;
        ...
        // progress, and
        // we are in stealth mode)
        boolean oldFlag = (paint.getFlags() & Paint.FILTER_BITMAP_FLAG) != 0;
        ...
        paint.setFilterBitmap(oldFlag); // restore default flag
    }
    ...
    private void drawArrow(Canvas canvas, float leftX, float topY, Cell start, Cell end)
    {
        ...
        final int offsetX = ((int) mSquareWidth - bitmapWidth) / 2;
        final int offsetY = ((int) mSquareHeight - bitmapHeight) / 2;
        ...
        Bitmap arrow = green ? bitmapArrowGreenUp : bitmapArrowRedUp;
        final int cellWidth = bitmapWidth;
        final int cellHeight = bitmapHeight;
        ...
        // compose matrix
        float sx = Math.min(mSquareWidth / bitmapWidth, 1.0f);
        float sy = Math.min(mSquareHeight / bitmapHeight, 1.0f);
        mArrowMatrix.setTranslate(leftX + offsetX, topY + offsetY); // transform
        // to cell
        // position
        mArrowMatrix.preTranslate(bitmapWidth / 2, bitmapHeight / 2);
        ...
        // 12:00
        // pos
        canvas.drawBitmap(arrow, mArrowMatrix, paint);
    }
    ...
    private void drawCircle(Canvas canvas, int leftX, int topY, boolean partOfPattern)
    {
        ...
        mCircleMatrix.preTranslate(bitmapWidth / 2, bitmapHeight / 2);
        mCircleMatrix.preScale(sx, sy);
        mCircleMatrix.preTranslate(-bitmapWidth / 2, -bitmapHeight / 2);

        canvas.drawBitmap(outerCircle, mCircleMatrix, paint);
        canvas.drawBitmap(innerCircle, mCircleMatrix, paint);
    }
    ...
    /**
     * Set the display mode of the current pattern. This can be useful, for instance, after detecting a pattern to tell this view whether change the in progress
     * result to correct or wrong.
     *
    ...
    public void setDisplayMode(DisplayMode displayMode)
    {
        ...
        if (displayMode == DisplayMode.Animate)
        {
            ...
            final Cell first = patternList.get(0);
    ...
    /**
     * Retrieves last display mode. This method is useful in case of storing states and restoring them after screen orientation changed.
     *
    ...
    /**
     * Retrieves current displaying pattern. This method is useful in case of storing states and restoring them after screen orientation changed.
     *
    ...
    {
        return (List<Cell>) patternList.clone();
    }
    ...
    {
        if (listener != null)
            listener.onPatternDetected(patternList);
    }
    ...
    /**
     * Determines whether the point x, y will add a new point to the current pattern (in addition to finding the cell, also makes heuristic choices such as
     * filling in gaps based on current pattern).
     *
    ...
    private void handleActionMove(MotionEvent event)
    {
        ...
        for (int i = 0; i < historySize + 1; i++)
        {
            ...
            final int patternSizePreHitDetect = patternList.size();
            Cell hitCell = detectAndAddHit(x, y);
            final int patternSize = patternList.size();
